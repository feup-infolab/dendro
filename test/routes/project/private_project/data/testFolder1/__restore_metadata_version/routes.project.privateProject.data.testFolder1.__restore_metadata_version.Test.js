...
describe('Private project testFolder1 level restore_metadata_version', function ()
{
    ...
    describe('[POST] [PRIVATE PROJECT] /project/' + privateProject.handle + '/data/foldername?restore_metadata_version', function ()
    {
        ...
        it('Should give an error of the request type for this route is html', function (done)
        {
            ...
            {
                itemUtils.itemRestoreMetadataVersion(false, agent, privateProject.handle, testFolder1.name, 0, function (err, res)
                {
        ...
        it('Should give an error if the user is unauthenticated', function (done)
        {
            ...
            itemUtils.itemRestoreMetadataVersion(true, agent, privateProject.handle, testFolder1.name, 0, function (err, res)
            {
        ...
        it('Should give an error if the metadata_version sent in the body is in an invalid format', function (done)
        {
            userUtils.loginUser(demouser1.username, demouser1.password, function (err, agent)
            {
                itemUtils.itemRestoreMetadataVersion(true, agent, privateProject.handle, testFolder1.name, 'thisisaninvalidversion', function (err, res)
                {
