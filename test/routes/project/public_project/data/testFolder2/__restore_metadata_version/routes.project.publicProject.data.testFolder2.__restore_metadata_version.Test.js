...
describe("Public project testFolder2 level restore_metadata_version", function ()
{
    ...
    describe("[POST] [PUBLIC PROJECT] /project/" + publicProject.handle + "/data/foldername?restore_metadata_version", function ()
    {
        ...
        it("Should give an error of the request type for this route is html", function (done)
        {
            ...
            {
                itemUtils.itemRestoreMetadataVersion(false, agent, publicProject.handle, testFolder2.name, 0, function (err, res)
                {
        ...
        it("Should give an error if the metadata_version sent in the body is in an invalid format", function (done)
        {
            userUtils.loginUser(demouser1.username, demouser1.password, function (err, agent)
            {
                itemUtils.itemRestoreMetadataVersion(true, agent, publicProject.handle, testFolder2.name, "thisisaninvalidversion", function (err, res)
                {
