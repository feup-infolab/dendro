                ...
                try
                {
                    userUtils.loginUser(demouser1.username, demouser1.password, function (err, agent) {
                        ...
                        projectUtils.getProjectRootContent(true, agent, metadataProject.handle, function (err, res)
                        {
                            should.equal(err, null);
                            // set here the testFolder1Data
                            metadataProjectTestFolder1Data = _.find(res.body, function (folder)
                            {
                                return folder.nie.title === testFolder1.name;
                            });
                            should.exist(metadataProjectTestFolder1Data);
                            metadataProjectRestoredFolderData = _.find(res.body, function (folder)
                            {
                                return folder.nie.title === restoredFolderName;
                            });
                            should.not.exist(metadataProjectRestoredFolderData);
                            callback(null);
                        });
                    });
                }
