...
const deleteFoldersUnit = require(Pathfinder.absPathInTestsFolder("units/folders/deleteFolders.Unit.js"));
const db = require(Pathfinder.absPathInTestsFolder("utils/db/db.Test.js"));

describe("Metadata only project testFolder1 level undelete tests", function ()
{
    ...
    describe("[POST] [METADATA ONLY PROJECT] /project/" + metadataProject.handle + "/data/:foldername?undelete", function ()
    {
        ...
        it("Should give an error when the request type for this route is HTML", function (done)
        {
            ...
            {
                itemUtils.undeleteItem(false, agent, metadataProject.handle, testFolder1.name, function (err, res)
                {
        ...
        it("Should give an error message when a project does not exist", function (done)
        {
            userUtils.loginUser(demouser1.username, demouser1.password, function (err, agent)
            {
                itemUtils.undeleteItem(true, agent, invalidProject.handle, testFolder1.name, function (err, res)
                {
        ...
        it("Should give an error when the user is not authenticated", function (done)
        {
            ...
            itemUtils.undeleteItem(true, agent, metadataProject.handle, testFolder1.name, function (err, res)
            {
