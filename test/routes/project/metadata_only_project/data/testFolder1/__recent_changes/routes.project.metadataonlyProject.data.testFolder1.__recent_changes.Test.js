...
describe("Metadata only project testFolder1 level recent changes", function () {
    before(function (done) {
        this.timeout(3*global.Config.testsTimeout);
    ...
    describe("[GET] [METADATA ONLY PROJECT] /project/"+ metadataProject.handle + "/data/foldername?recent_changes", function () {
        //API ONLY
        it("Should give an error if the request is of type HTML", function (done) {
            userUtils.loginUser(demouser1.username, demouser1.password, function (err, agent) {
                itemUtils.getItemRecentChanges(false, agent, metadataProject.handle, testFolder1.name, function (err, res) {
        ...
        it("Should give an error if the user is unauthenticated", function (done) {
            ...
            itemUtils.getItemRecentChanges(true, agent, metadataProject.handle, testFolder1.name, function (err, res) {
        ...
        it("Should give an error if the folder does not exist", function (done) {
            userUtils.loginUser(demouser1.username, demouser1.password, function (err, agent) {
                itemUtils.getItemRecentChanges(true, agent, metadataProject.handle, notFoundFolder.name, function (err, res) {
                    res.statusCode.should.equal(404);
                    done();
                });
            });
        ...
        it("Should give the folder changes if the user is logged in as demouser2(a collaborator on the project)", function (done) {
            userUtils.loginUser(demouser2.username, demouser2.password, function (err, agent) {
                //jsonOnly, agent, projectHandle, itemPath, cb
                itemUtils.getItemRecentChanges(true, agent, metadataProject.handle, folderForDemouser2.name, function (err, res) {
                    res.statusCode.should.equal(200);
                    res.body[0].changes.length.should.equal(3);//The abstract, title and creator descriptors
                    done();
                });
