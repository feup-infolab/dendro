...
describe("Metadata only project testFolder2 level parent_metadata tests", function ()
{
    ...
    describe("/project/" + metadataProject.handle + "/data/" + testFolder2.name + "?parent_metadata (metadata only project)", function ()
    {
        /**
         ...
         * Valid request type
         */
        it("[JSON] should refuse to fetch the parent_metadata of the " + metadataProject.handle + "/data/" + testFolder2.name + " resource without authenticating", function (done)
        {
            const app = global.tests.app;
            const agent = chai.request.agent(app);
            itemUtils.getItemParentMetadata(true, agent, metadataProject.handle, testFolder2.name, function (err, res)
            {
        ...
        it("[JSON] should fetch the parent_metadata of the " + metadataProject.handle + "/data/" + testFolder2.name + " resource, authenticated as " + demouser1.username + " (creator)", function (done)
        {
            userUtils.loginUser(demouser1.username, demouser1.password, function (err, agent)
            {
                itemUtils.getItemParentMetadata(true, agent, metadataProject.handle, testFolder2.name, function (err, res)
        ...
        it("[JSON] should refuse to fetch the parent_metadata of the " + metadataProject.handle + "/data/" + testFolder2.name + " resource, authenticated as " + demouser3.username + " (not creator nor contributor)", function (done)
        {
        ...
        it("[JSON] should fetch the parent_metadata of the " + metadataProject.handle + "/data/" + testFolder2.name + " resource, authenticated as " + demouser2.username + " (contributor)", function (done)
        {
