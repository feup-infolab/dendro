...
describe("Export metadata only project folderExportCkan level to ckan tests", function ()
{
    ...
    before(function (done)
    {
        ...
        addChangesToExportedCkanPackagesUnit.init(metadataOnlyProject, function (err, results)
        {
            ...
            repositoryUtils.getMyExternalRepositories(true, agent, function (err, res)
            {
                ...
                projectUtils.getProjectRootContent(true, agent, metadataOnlyProject.handle, function (err, res)
                {
                    res.statusCode.should.equal(200);
    ...
    describe("[POST] [CKAN] /project/:handle/data/:foldername?export_to_repository", function ()
    {
        ...
        it.optional("Should export the folder  to ckan even when uploading a file with a size of zero to Dendro (this use case caused a bug before, now dendro does not accept files with a size of zero)", function (done)
        {
            ...
            userUtils.loginUser(demouser1.username, demouser1.password, function (err, agent)
            {
                fileUtils.uploadFile(true, agent, metadataOnlyProject.handle, folderExportedCkanDendroDiffsData.nie.title, emptyFileMock, function (err, res)
                {
                    res.statusCode.should.equal(200);
        ...
        it.optional("Should append the current date if a file with the same name was already uploaded before, however, should not export because the user did not allow dendroPermissions", function (done)
        {
            ...
            userUtils.loginUser(demouser1.username, demouser1.password, function (err, agent)
            {
                ...
                fileUtils.uploadFile(true, agent, metadataOnlyProject.handle, folderExportedCkanDendroDiffsData.nie.title, uploadedAndDeletedFileInDendroMockFile, function (err, res)
                {
                    res.statusCode.should.equal(200);
        ...
        it.optional("Should give an error saying that a folder has no content to export", function (done)
        {
            ...
            userUtils.loginUser(demouser1.username, demouser1.password, function (err, agent)
            {
                projectUtils.createFolderInProjectRoot(true, agent, metadataOnlyProject.handle, "anEmptyFolder", function (err, res)
                {
                    res.statusCode.should.equal(200);
        ...
        it.optional("Should give an error saying that a folder to export has children folders(ckan does not support this)", function (done)
        {
            ...
            userUtils.loginUser(demouser1.username, demouser1.password, function (err, agent)
            {
                projectUtils.createFolderInProjectRoot(true, agent, metadataOnlyProject.handle, "aFolderWithFolders", function (err, res)
                {
                    res.statusCode.should.equal(200);
                    aFolderWithFoldersUri = res.body.id;
                    itemUtils.createFolder(true, agent, metadataOnlyProject.handle, "aFolderWithFolders", "folderInsideAFolder", function (err, res)
                    {
                        res.statusCode.should.equal(200);
