...
describe("B2Drop project testFolder1 ?rename", function ()
{
    ...
    describe("[POST] [FOLDER] [B2Drop PROJECT] [Invalid Cases] /project/" + b2dropProject.handle + "/data/:foldername?rename", function ()
    {
        ...
        it("Should upload a dummy text file into the folder testFolder1", function (done)
        {
            ...
            {
                should.equal(err, null);
                fileUtils.uploadFileToProjectRoot(true, agent, b2dropProject.handle, txtMockFile, function (err, res)
                {
                    res.statusCode.should.equal(200);
                    fileUtils.downloadFileFromProjectRoot(true, agent, b2dropProject.handle, txtMockFile, function (error, response)
                    {
                        should.equal(err, null);
        ...
        it("Should be unable to find the file if we try to rename a file that does not exist, even if the user is logged in as a creator or collaborator on the project", function (done)
        {
            userUtils.loginUser(demouser2.username, demouser2.password, function (err, agent)
            {
                async.mapSeries(allFiles, function (file, callback)
                {
                    ...
                    fileUtils.renameFile(agent, b2dropProject.handle, testFolder1.name, inexistentFileName, newName, function (err, res)
                    {
                        res.statusCode.should.equal(200);
        ...
        it("Should give an error if we try to rename a project instead of a file, even if the user is logged in as a creator or collaborator on the project", function (done)
        {
            userUtils.loginUser(demouser2.username, demouser2.password, function (err, agent)
            {
                ...
                {
                    const newName = "RenamedFile";
                    fileUtils.renameProject(true, agent, b2dropProject.handle, newName, function (err, res)
                    {
    ...
    describe("[POST] [FILE] [B2Drop PROJECT] [Valid Cases] /project/" + b2dropProject.handle + "/data/testFolder1/:filename?rename", function ()
    {
        ...
        it("Should rename files with success if the user is logged in as demouser1(the creator of the project)", function (done)
        {
            userUtils.loginUser(demouser1.username, demouser1.password, function (err, agent)
            {
                async.mapSeries(allFiles, function (file, callback)
                {
                    const newName = "RenamedFile" + file.extension;
                    fileUtils.renameFile(agent, b2dropProject.handle, testFolder1.name, file.name, newName, function (err, res)
                    {
                        res.statusCode.should.equal(200);
                        callback(err, res);
