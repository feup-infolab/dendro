...
describe("Get the shares of a specific post tests", function () {
    ...
    describe("[POST] Get the shares of a specific post /posts/shares", function () {

        it("[For an unauthenticated user] Should give an unauthorized error", function (done) {
            done();
        });

        it("[For demouser1, as the creator of all projects] Should give zero shares for an existing post(a post that was not shared) in a project created by demouser1", function (done) {
            done();
        });

        it("[For demouser1, as the creator of all projects] Should give one share for a post that was shared only once in a project created by demouser1", function (done) {
            done();
        });

        it("[For demouser2, a collaborator in all projects] Should give one share for a post that was shared only once in a project where demouser2 collaborates", function (done) {
            done();
        });
