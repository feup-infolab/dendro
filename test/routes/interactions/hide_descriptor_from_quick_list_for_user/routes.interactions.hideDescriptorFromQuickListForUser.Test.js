...
/* let bodyObj = {
    ...
    "rankingPosition": 2,
    "pageNumber": 0,
    "interactionType": "hide_descriptor_from_quick_list_for_user",
...
describe("[" + publicProject.handle + "]" + "[INTERACTION TESTS] hide_descriptor_from_quick_list_for_user", function ()
{
            ...
            userUtils.loginUser(demouser1.username, demouser1.password, function (err, agent)
            {
                ...
                projectUtils.getProjectRootContent(true, agent, publicProject.handle, function (err, res)
                {
                    ...
                    descriptorUtils.getDescriptorsFromOntology(true, agent, dctermsPrefix, function (err, res)
                    {
                        ...
                        demouser1InteractionObj.interactionType = "hide_descriptor_from_quick_list_for_user";
                        ...
                        demouser2InteractionObj.interactionType = "hide_descriptor_from_quick_list_for_user";
    ...
    describe("[POST] [Invalid Cases] /interactions/hide_descriptor_from_quick_list_for_user", function ()
    {
        it("Should give an error and not accept and register an interaction when a descriptor is hidden from the quick list when unauthenticated", function (done)
        {
            const app = global.tests.app;
            let agent = chai.request.agent(app);
            interactionsUtils.hideDescriptorFromQuickListForUser(true, agent, demouser1InteractionObj, function (err, res)
            {
                should.exist(err);
        ...
        it("Should give an error and not accept and register an interaction when a descriptor is hidden from the quick list when logged in as demouser3, who is not a creator or collaborator of the project where the current resource belongs to", function (done)
        {
        ...
        it("Should give an error and not accept and register an interaction if the interaction type field is invalid, even if logged in as demouser1 (creator of the project)", function (done)
        {
            userUtils.loginUser(demouser1.username, demouser1.password, function (err, agent)
            {
                ...
                interactionsUtils.hideDescriptorFromQuickListForUser(true, agent, copyOfDemouser1InteractionObj, function (err, res)
                {
                    should.exist(err);
                    should.exist(res);
                    res.statusCode.should.equal(500);
                    res.body.message.should.equal("Invalid interaction type in the request's body. It should be : hide_descriptor_from_quick_list_for_user");
                    // SHOULD NOT BE IN THE MYSQL DATABASE
                    interactionsUtils.getNumberOfInteractionsInDB(function (err, info)
                    {
                        should.equal(err, null);
        ...
        it("Should give an error and not accept and register an interaction if the recommendationCallId field is missing, even if logged in as demouser1 (creator of the project)", function (done)
        {
            userUtils.loginUser(demouser1.username, demouser1.password, function (err, agent)
            {
                ...
                interactionsUtils.hideDescriptorFromQuickListForUser(true, agent, copyOfDemouser1InteractionObj, function (err, res)
                {
                    ...
                    should.exist(res);
                    res.statusCode.should.equal(500);
                    res.body.message.should.equal("Interaction type hide_descriptor_from_quick_list_for_user requires field recommendationCallId in the request's body.");
                    // SHOULD NOT BE IN THE MYSQL DATABASE
                    interactionsUtils.getNumberOfInteractionsInDB(function (err, info)
                    {
                        should.equal(err, null);
    ...
    describe("[POST] [Valid Cases] /interactions/hide_descriptor_from_quick_list_for_user", function ()
    {
                        ...
                        interactionsUtils.getNumberOfInteractionsInDB(function (err, info)
                        {
                            ...
                            interactionsUtils.getLatestInteractionInDB(function (err, info)
                            {
                                ...
                                descriptorUtils.getResourceDescriptorsFromOntology(true, agent, demouser1InteractionObj.recommendedFor, dctermsPrefix, function (err, res)
                                {
                                    ...
                                    res.body.descriptors[demouser1DescriptorIndex].recommendation_types.user_hidden.should.equal(true);
                                    ...
                                    should.equal(res.body.descriptors[demouser2DescriptorIndex].recommendation_types.user_hidden, undefined);
                                    done();
                                });
                            });
                        });
        ...
        it("Should accept and register an interaction when a descriptor is added from  the manual list while it was a project and user favorite when logged in as demouser2 (a collaborator on the current project)", function (done)
        {
            userUtils.loginUser(demouser2.username, demouser2.password, function (err, agent)
            {
                userUtils.getUserInfo(demouser2.username, true, agent, function (err, res)
                {
                    ...
                    interactionsUtils.hideDescriptorFromQuickListForUser(true, agent, demouser2InteractionObj, function (err, res)
                    {
                        should.equal(err, null);
                        res.statusCode.should.equal(200);
                        // SHOULD ALSO BE IN THE MYSQL DATABASE
                        interactionsUtils.getNumberOfInteractionsInDB(function (err, info)
                        {
                            ...
                            interactionsUtils.getLatestInteractionInDB(function (err, info)
                            {
                                ...
                                descriptorUtils.getResourceDescriptorsFromOntology(true, agent, demouser1InteractionObj.recommendedFor, dctermsPrefix, function (err, res)
                                {
                                    ...
                                    expect(demouser1DescriptorIndex).to.be.above(-1);
                                    expect(res.body.descriptors.length).to.be.above(demouser1DescriptorIndex);
                                    // because this descriptor was only hidden for demouser1 and the logged in user in this test is the demouser2
                                    should.equal(res.body.descriptors[demouser1DescriptorIndex].recommendation_types.user_hidden, undefined);
                                    ...
                                    demouser2DescriptorIndex.should.not.equal(demouser1DescriptorIndex);
                                    expect(res.body.descriptors.length).to.be.above(demouser2DescriptorIndex);
                                    // because this descriptor was hidden only for demouser2 and the logged user in this test is the demouser2
                                    res.body.descriptors[demouser2DescriptorIndex].recommendation_types.user_hidden.should.equal(true);
                                    done();
                                });
                            });
                        });
