...
describe("[" + publicProject.handle + "]" + "[INTERACTION TESTS] fill_in_descriptor_from_manual_list_while_it_was_a_user_favorite", function ()
{
            ...
            userUtils.loginUser(demouser1.username, demouser1.password, function (err, agent)
            {
                ...
                projectUtils.getProjectRootContent(true, agent, publicProject.handle, function (err, res)
                {
                    ...
                    descriptorUtils.getDescriptorsFromOntology(true, agent, dctermsPrefix, function (err, res)
                    {
                        ...
                        demouser1InteractionObj.interactionType = "fill_in_descriptor_from_manual_list_while_it_was_a_user_favorite";
                        ...
                        demouser2InteractionObj.interactionType = "fill_in_descriptor_from_manual_list_while_it_was_a_user_favorite";
    ...
    describe("[POST] [Invalid Cases] /interactions/fill_in_descriptor_from_manual_list_while_it_was_a_user_favorite", function ()
    {
        it("Should give an error and not accept and register an interaction when a descriptor is filled in from the manual list while it was a user favorite when unauthenticated", function (done)
        {
            const app = global.tests.app;
            agent = chai.request.agent(app);
            interactionsUtils.fillInDescriptorFromManualListWhileItWasAUserFavorite(true, agent, demouser1InteractionObj, function (err, res)
            {
                should.exist(err);
        ...
        it("Should give an error and not accept and register an interaction when a descriptor is filled in from the manual list while it was a user favorite when logged in as demouser3, who is not a creator or collaborator of the project where the current resource belongs to", function (done)
        {
        ...
        it("Should give an error and not accept and register an interaction if the interaction type field is invalid, even if logged in as demouser1 (creator of the project)", function (done)
        {
            userUtils.loginUser(demouser1.username, demouser1.password, function (err, agent)
            {
                ...
                interactionsUtils.fillInDescriptorFromManualListWhileItWasAUserFavorite(true, agent, copyOfDemouser1InteractionObj, function (err, res)
                {
                    should.exist(err);
                    should.exist(res);
                    res.statusCode.should.equal(500);
                    res.body.message.should.equal("Invalid interaction type in the request's body. It should be : fill_in_descriptor_from_manual_list_while_it_was_a_user_favorite");
                    // SHOULD NOT BE IN THE MYSQL DATABASE
                    interactionsUtils.getNumberOfInteractionsInDB(function (err, info)
                    {
                        should.equal(err, null);
        ...
        it("Should give an error and not accept and register an interaction if the recommendationCallId field is missing, even if logged in as demouser1 (creator of the project)", function (done)
        {
            userUtils.loginUser(demouser1.username, demouser1.password, function (err, agent)
            {
                ...
                interactionsUtils.fillInDescriptorFromManualListWhileItWasAUserFavorite(true, agent, copyOfDemouser1InteractionObj, function (err, res)
                {
                    ...
                    should.exist(res);
                    res.statusCode.should.equal(500);
                    res.body.message.should.equal("Interaction type fill_in_descriptor_from_manual_list_while_it_was_a_user_favorite requires field recommendationCallId in the request's body.");
                    // SHOULD NOT BE IN THE MYSQL DATABASE
                    interactionsUtils.getNumberOfInteractionsInDB(function (err, info)
                    {
                        should.equal(err, null);
    ...
    describe("[POST] [Valid Cases] /interactions/fill_in_descriptor_from_manual_list_while_it_was_a_user_favorite", function ()
    {
        it("Should accept and register an interaction when a descriptor is filled in from the manual list while it was a user favorite when logged in as demouser1 (the creator of the current project)", function (done)
        {
            userUtils.loginUser(demouser1.username, demouser1.password, function (err, agent)
            {
        ...
        it("Should accept and register an interaction when a descriptor is filled in from the manual list while it was a user favorite when logged in as demouser2 (a collaborator on the current project)", function (done)
        {
            userUtils.loginUser(demouser2.username, demouser2.password, function (err, agent)
            {
                userUtils.getUserInfo(demouser2.username, true, agent, function (err, res)
                {
                    ...
                    interactionsUtils.fillInDescriptorFromManualListWhileItWasAUserFavorite(true, agent, demouser2InteractionObj, function (err, res)
                    {
                        should.equal(err, null);
                        res.statusCode.should.equal(200);
                        // SHOULD ALSO BE IN THE MYSQL DATABASE
