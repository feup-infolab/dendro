...
/*let bodyObj = {
    ...
    "rankingPosition": 2,
    "pageNumber": 0,
    "interactionType": "accept_descriptor_from_autocomplete",
...
describe("[" + publicProject.handle + "]"   + "[INTERACTION TESTS] accept_descriptor_from_autocomplete", function ()
{
            ...
            userUtils.loginUser(demouser1.username, demouser1.password, function (err, agent)
            {
                ...
                projectUtils.getProjectRootContent(true, agent, publicProject.handle, function (err, res)
                {
                    ...
                    descriptorUtils.getDescriptorsFromOntology(true, agent, dctermsPrefix, function (err, res) {
                        ...
                        demouser1InteractionObj.interactionType = "accept_descriptor_from_autocomplete";
                        ...
                        demouser2InteractionObj.interactionType = "accept_descriptor_from_autocomplete";
    ...
    describe("[POST] [Invalid Cases] /interactions/accept_descriptor_from_autocomplete", function ()
    {
        it("Should give an error and not accept and register an interaction when a descriptor is accepted from the autocomplete feature when unauthenticated", function (done)
        {
            const app = global.tests.app;
            let agent = chai.request.agent(app);
            interactionsUtils.acceptDescriptorFromAutocomplete(true, agent, demouser1InteractionObj, function (err, res)
            {
                should.exist(err);
        ...
        it("Should give an error and not accept and register an interaction when a descriptor is accepted from the autocomplete feature when logged in as demouser3, who is not a creator or collaborator of the project where the current resource belongs to", function (done)
        {
        ...
        it("Should give an error and not accept and register an interaction if the interaction type field is invalid, even if logged in as demouser1 (creator of the project)", function (done)
        {
            userUtils.loginUser(demouser1.username, demouser1.password, function (err, agent)
            {
                ...
                interactionsUtils.acceptDescriptorFromAutocomplete(true, agent, copyOfDemouser1InteractionObj, function (err, res)
                {
                    should.exist(err);
                    should.exist(res);
                    res.statusCode.should.equal(500);
                    res.body.message.should.equal("Invalid interaction type in the request's body. It should be : accept_descriptor_from_autocomplete");
                    //SHOULD NOT BE IN THE MYSQL DATABASE
                    interactionsUtils.getNumberOfInteractionsInDB(function (err, info) {
                        should.equal(err, null);
                        should.exist(info);
        ...
        it("Should give an error and not accept and register an interaction if the recommendationCallId field is missing, even if logged in as demouser1 (creator of the project)", function (done)
        {
            userUtils.loginUser(demouser1.username, demouser1.password, function (err, agent)
            {
                ...
                interactionsUtils.acceptDescriptorFromAutocomplete(true, agent, copyOfDemouser1InteractionObj, function (err, res)
                {
                    ...
                    should.exist(res);
                    res.statusCode.should.equal(500);
                    res.body.message.should.equal("Interaction type accept_descriptor_from_autocomplete requires field recommendationCallId in the request's body.");
                    //SHOULD NOT BE IN THE MYSQL DATABASE
                    interactionsUtils.getNumberOfInteractionsInDB(function (err, info) {
                        should.equal(err, null);
                        should.exist(info);
    ...
    describe("[POST] [Valid Cases] /interactions/accept_descriptor_from_autocomplete", function ()
    {
        it("Should accept and register an interaction when a descriptor is accepted from the autocomplete feature when logged in as demouser1 (the creator of the current project)", function (done)
        {
            userUtils.loginUser(demouser1.username, demouser1.password, function (err, agent)
            {
        ...
        it("Should accept and register an interaction when a descriptor is accepted from the autocomplete feature when logged in as demouser2 (a collaborator on the current project)", function (done)
        {
            userUtils.loginUser(demouser2.username, demouser2.password, function (err, agent)
            {
                userUtils.getUserInfo(demouser2.username, true, agent, function (err, res)
                {
                    ...
                    interactionsUtils.acceptDescriptorFromAutocomplete(true, agent, demouser2InteractionObj, function (err, res)
                    {
                        should.equal(err, null);
                        res.statusCode.should.equal(200);
                        //SHOULD ALSO BE IN THE MYSQL DATABASE
