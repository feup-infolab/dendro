...
/* let bodyObj = {
    ...
    "rankingPosition": 2,
    "pageNumber": 0,
    "interactionType": "delete_descriptor_in_metadata_editor",
...
describe("[" + publicProject.handle + "]" + "[INTERACTION TESTS] delete_descriptor_in_metadata_editor", function ()
{
            ...
            userUtils.loginUser(demouser1.username, demouser1.password, function (err, agent)
            {
                ...
                projectUtils.getProjectRootContent(true, agent, publicProject.handle, function (err, res)
                {
                    ...
                    descriptorUtils.getDescriptorsFromOntology(true, agent, dctermsPrefix, function (err, res)
                    {
                        ...
                        demouser1InteractionObj.interactionType = "delete_descriptor_in_metadata_editor";
                        ...
                        demouser2InteractionObj.interactionType = "delete_descriptor_in_metadata_editor";
    ...
    describe("[POST] [Invalid Cases] /interactions/delete_descriptor_in_metadata_editor", function ()
    {
        ...
        it("Should give an error and not accept and register an interaction when deleting a descriptor in the metadata editor when logged in as demouser3, who is not a creator or collaborator of the project where the current resource belongs to", function (done)
        {
        ...
        it("Should give an error and not accept and register an interaction if the interaction type field is invalid, even if logged in as demouser1 (creator of the project)", function (done)
        {
            userUtils.loginUser(demouser1.username, demouser1.password, function (err, agent)
            {
                ...
                interactionsUtils.deleteDescriptorInMetadataEditor(true, agent, copyOfDemouser1InteractionObj, function (err, res)
                {
                    should.exist(err);
                    should.exist(res);
                    res.statusCode.should.equal(500);
                    res.body.message.should.equal("Invalid interaction type in the request's body. It should be : delete_descriptor_in_metadata_editor");
                    // SHOULD NOT BE IN THE MYSQL DATABASE
                    interactionsUtils.getNumberOfInteractionsInDB(function (err, info)
                    {
                        should.equal(err, null);
        ...
        it("Should give an error and not accept and register an interaction if the recommendationCallId field is missing, even if logged in as demouser1 (creator of the project)", function (done)
        {
            userUtils.loginUser(demouser1.username, demouser1.password, function (err, agent)
            {
                ...
                interactionsUtils.deleteDescriptorInMetadataEditor(true, agent, copyOfDemouser1InteractionObj, function (err, res)
                {
                    ...
                    should.exist(res);
                    res.statusCode.should.equal(500);
                    res.body.message.should.equal("Interaction type delete_descriptor_in_metadata_editor requires field recommendationCallId in the request's body.");
                    // SHOULD NOT BE IN THE MYSQL DATABASE
                    interactionsUtils.getNumberOfInteractionsInDB(function (err, info)
                    {
                        should.equal(err, null);
    ...
    describe("[POST] [Valid Cases] /interactions/delete_descriptor_in_metadata_editor", function ()
    {
        it("Should accept and register an interaction when deleting a descriptor in the metadata editor when logged in as demouser1 (the creator of the current project)", function (done)
        {
            userUtils.loginUser(demouser1.username, demouser1.password, function (err, agent)
            {
        ...
        it("Should accept and register an interaction when deleting a descriptor in the metadata editor when logged in as demouser2 (a collaborator on the current project)", function (done)
        {
            userUtils.loginUser(demouser2.username, demouser2.password, function (err, agent)
            {
                userUtils.getUserInfo(demouser2.username, true, agent, function (err, res)
                {
                    ...
                    interactionsUtils.deleteDescriptorInMetadataEditor(true, agent, demouser2InteractionObj, function (err, res)
                    {
                        should.equal(err, null);
                        res.statusCode.should.equal(200);
                        // SHOULD ALSO BE IN THE MYSQL DATABASE
