...
docker run --name elasticsearch-dendro \
    ...
    -d elasticsearch:2.4.6 1> /dev/null || docker start elasticsearch-dendro 1> /dev/null || docker restart elasticsearch-dendro 1> /dev/null
...
docker run --name virtuoso-dendro \
    ...
    -d tenforce/virtuoso:1.2.0-virtuoso7.2.4 1> /dev/null || docker start virtuoso-dendro 1> /dev/null || docker restart virtuoso-dendro 1> /dev/null

docker run --name mysql-dendro \
    -p 3306:3306 \
    -e MYSQL_ROOT_PASSWORD=r00t \
    -v "$RUNNING_FOLDER/mysql:/var/lib/mysql" \
    -d mysql:8.0.3 1> /dev/null || docker start mysql-dendro 1> /dev/null || docker restart mysql-dendro 1> /dev/null

docker run --name mongo-dendro \
    -p 27017:27017 \
    -v "$RUNNING_FOLDER/mongo:/data/db" \
    -d mongo:3.4.10 1> /dev/null || docker start mongo-dendro 1> /dev/null || docker restart mongo-dendro 1> /dev/null
