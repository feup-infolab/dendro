...
docker run --name elasticsearch-dendro \
    ...
    -d elasticsearch:2.3.3-dendro-v0.3 || docker start elasticsearch-dendro

docker run --name mysql-dendro \
    -p 3306:3306 \
    -v $RUNNING_FOLDER/mysql:/var/lib/mysql \
    -d mysql:8.0.3-dendro-v0.3 || docker start mysql-dendro


docker run --name mongo-dendro \
    -p 27017:27017 \
    -v $RUNNING_FOLDER/mongo:/data/db \
    -d mongo:3.4.10-dendro-v0.3 || docker start mongo-dendro


docker run --name redis-dendro-default \
    -p 6781:6780 \
    -v $RUNNING_FOLDER/redis-default:/data \
    -d redis:3.2.11-dendro-v0.3 || docker start redis-dendro-default

docker run --name redis-dendro-social \
    -p 6782:6780 \
    -v $RUNNING_FOLDER/redis-default:/data \
    -d redis:3.2.11-dendro-v0.3 || docker start redis-dendro-social

docker run --name redis-dendro-notifications \
    -p 6783:6780 \
    -v $RUNNING_FOLDER/redis-default:/data \
    -d redis:3.2.11-dendro-v0.3 || docker start redis-dendro-notifications
